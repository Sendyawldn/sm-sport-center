...
export default function BookingPage() {
  ...
  const handleConfirmBooking = async () => {
    ...
    const res = await createBooking(selectedSlot.court.id, selectedDate, selectedSlot.time, 1);
    setIsBooking(false);
    
    if (res.error) {
      setMessage({ text: res.error, type: "error" });
    } else {
      setMessage({ text: "Booking berhasil ditahan selama 15 menit. Silakan lakukan pembayaran.", type: "success" });
      setSelectedSlot(null);
      // Refresh
      setLoading(true);
      const data = await fetch(`/api/courts?date=${selectedDate}`).then(r => r.json());
      if (data.courts) setCourts(data.courts);
      if (data.bookings) setBookings(data.bookings);
      setLoading(false);
    }
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-10 px-4 sm:px-6 lg:px-8 font-sans">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-3xl font-bold text-gray-900 mb-8">Kalender Jadwal Lapangan</h1>
        ...
        <div className="mb-6 flex overflow-x-auto gap-2 pb-2">
          {dateOptions.map(dateStr => (
            <button
              ...
              className={`flex-shrink-0 px-5 py-3 rounded-xl border ${
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-x-auto">
          {loading ? (
            <div className="p-20 text-center text-gray-500">Memuat jadwal...</div>
          ) : (
            <table className="w-full text-left border-collapse">
              <thead>
                <tr>
                  ...
                  {courts.map(court => (
                    <th key={court.id} className="p-4 border-b border-gray-200 bg-gray-50 font-semibold text-gray-700 min-w-[150px]">
                      {court.name}
                      <div className="text-xs font-normal text-gray-500 mt-1">Rp {court.pricePerHour.toLocaleString('id-ID')} / jam</div>
                    </th>
                  ))}
                </tr>
              </thead>
                  ...
                  <tr key={hour} className="group">
                    ...
                    {courts.map(court => {
                      ...
                      if (status === "PAST") {
                        ...
                        bgClass = "bg-green-50 hover:bg-green-100 transition-colors";
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl max-w-md w-full overflow-hidden">
            <div className="p-6 border-b border-gray-100">
              <h3 className="text-xl font-bold text-gray-900">Konfirmasi Booking</h3>
            </div>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="flex justify-between pb-3 border-b border-gray-100">
                ...
                <span className="font-semibold text-gray-900">{selectedSlot.court.name}</span>
              </div>
              <div className="flex justify-between pb-3 border-b border-gray-100">
                <span className="text-gray-500">Tanggal</span>
                <span className="font-semibold text-gray-900">{format(parseISO(selectedDate), 'dd MMMM yyyy')}</span>
              </div>
              <div className="flex justify-between pb-3 border-b border-gray-100">
                <span className="text-gray-500">Waktu</span>
                <span className="font-semibold text-gray-900">{selectedSlot.time} - {`${parseInt(selectedSlot.time.split(':')[0]) + 1}:00`}</span>
              </div>
              <div className="flex justify-between pb-3">
                <span className="text-gray-500">Total Harga</span>
                <span className="font-bold text-blue-600 text-lg">Rp {selectedSlot.court.pricePerHour.toLocaleString('id-ID')}</span>
              </div>
            </div>
            
            <div className="p-6 bg-gray-50 flex gap-3">
              <button 
                onClick={() => { setSelectedSlot(null); setMessage(null); }}
