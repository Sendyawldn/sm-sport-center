...
function FilterBarContent({ className }: { className: string }) {
  ...
  return (
    <div className={`w-full max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative z-20 ${className}`}>
      {sportOpen && (
      ...
      <div className="bg-[#a6192e] rounded-xl shadow-xl w-full relative z-50">
        <form onSubmit={handleSearch} className="flex flex-col md:flex-row items-center justify-between p-4 md:p-5 gap-4 md:gap-0">
            ...
            >
              ...
              {sportOpen && (
                <div className="absolute top-16 left-0 w-64 bg-white rounded-xl shadow-2xl py-2 z-50 border border-gray-100 overflow-hidden">
                  <div className="px-6 py-3 hover:bg-gray-50 text-gray-800 hover:text-[#0056b3] text-[17px] cursor-pointer transition-colors" onClick={() => setSport("")}>Semua Olahraga</div>
                  <div className="px-6 py-3 hover:bg-gray-50 text-gray-800 hover:text-[#0056b3] text-[17px] cursor-pointer transition-colors" onClick={() => setSport("futsal")}>Futsal</div>
                  <div className="px-6 py-3 hover:bg-gray-50 text-gray-800 hover:text-[#0056b3] text-[17px] cursor-pointer transition-colors" onClick={() => setSport("badminton")}>Badminton</div>
                </div>
              )}
            </div>
          ...
          <div className="w-full md:w-auto md:ml-4 flex-shrink-0">
            <button type="submit" className="w-full md:w-auto bg-white text-[#a6192e] hover:bg-gray-100 font-bold text-[15px] py-3 px-8 rounded-full flex items-center justify-center gap-2 transition-transform active:scale-95 shadow-sm">
              Temukan
...
export default function FilterBar({ className = "-mt-10 sm:-mt-14" }: { className?: string }) {
  return (
    <Suspense fallback={<div className={`w-full max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 h-24 bg-[#a6192e] rounded-xl ${className}`}></div>}>
